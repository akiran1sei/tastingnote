...
export async function GET(req, res) {
  ...
  try {
    ...
    console.error("PDF作成中にエラーが発生しました:", error); // エラーの種類に応じて、より具体的なエラーメッセージを返す
    ...
    if (error instanceof Error && error.message.includes("Network")) {
      return NextResponse.json({
        ...
        error: error.stack, // スタックトレースをログに出力
      });
    } else if (error instanceof TemplateRenderError) {
      // カスタムエラー

      return NextResponse.json({
        message: `テンプレートレンダリングエラーが発生しました: ${error.message}`,
      ...
      console.error("予期しないエラーが発生しました:", error); // ユーザーに表示するエラーメッセージ
